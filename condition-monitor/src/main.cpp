...
// TODO: Move these to a .json config file or environment variables for dynamic
// scaling
} // namespace config
...
private:
  ...
  bool performPOST() {
    // Power-On Self Test: Simulating RAM/Flash integrity check (Safety Req
    // SR-03)
...
public:
  SafetyMonitorApp(std::string id)
      : nodeId(id), filter(5), stateMachine(200.0f, 400.0f),
        aiModel(0.005f, 50, 20) {
  ...
  void run() {
    ...
    while (true) {
      ...
      if (!isSystemInSafeMode) {
        ...
        if (sensor.readRegister(0x00, rawData, 2)) {
          ...
          if (lastCanBroadcast >= config::CAN_BROADCAST_RATE_MS) {
            ...
            canData[2] =
                (stateMachine.getStateName() == "NORMAL")                ? 0
                : (stateMachine.getStateName() == "WARNING")             ? 1
                : (stateMachine.getStateName() == "PREDICTIVE_CRITICAL") ? 3
                                                                         : 2;
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(config::SAMPLE_RATE_MS));
